...
function transformToLis(obj){
  ...
  const list = keys.map(key => `<li>${key}: ${obj[key]}</li>`)
  return list;
}
...
let findShortest = (data) => {
  // Solution code here...
  let shortestCharacter = data.reduce((prev, current) => {
    return parseInt(current.height) < parseInt(prev.height) ? current : prev;
  });
